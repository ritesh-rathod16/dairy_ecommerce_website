    ...
    <div className="flex min-h-screen items-center justify-center bg-ink px-4">
      <div className="w-full max-w-sm">
        <div className="mb-6 text-center">
          ...
          <h1 className="mt-2 font-display text-2xl font-semibold text-cream">Katlkar Dairy — Admin</h1>
          <p className="mt-1 text-sm text-cream/50">Staff sign-in only</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-4 rounded-xl2 bg-white p-6 shadow-lg">
          <div>
            <label className="mb-1 block text-sm font-medium text-ink/70">Admin email</label>
            <input
        ...
        <p className="mt-4 text-center text-sm text-cream/50">
          Not staff? <Link to="/login" className="font-semibold text-turmeric underline">Go to customer login</Link>
        </p>
      </div>
    </div>
  );
